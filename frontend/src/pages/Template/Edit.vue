    ...
    <div class="editor-container">
      ...
      <div class="preview-section">
        <t-card title="实时预览" class="preview-card">
          <ResumeRenderer
            :resume-data="{ content: form.content, title: form.name }"
            :template-config="{
              layout: form.layout,
...
const handleSave = async () => {
  try {
    // 处理项目经历的技术栈（将字符串转换为数组）
    const processedContent = JSON.parse(JSON.stringify(form.value.content))
    if (processedContent.projects) {
      processedContent.projects = processedContent.projects.map(proj => ({
        ...proj,
        technologies: proj.technologiesString ? proj.technologiesString.split(',').map(t => t.trim()) : []
      }))
    }
